...
def platform_color_badge(name:str)->str:
    ...
    for k,c in PLATFORM_COLOR.items():
        if k in key: return f'<span style="color:{c};font-weight:bold">{k.capitalize()}</span>'
...
def trim_columns(df:pd.DataFrame,needed:set[str])->pd.DataFrame:
...
def parse_dates_series(s:pd.Series):
    def norm(x:str):
        x=str(x)
        if " " in x:
            d,t=x.split(" ",1); t=t.replace(".",":"); x=f"{d} {t}"
        return x
    return pd.to_datetime(s.astype(str).map(norm),dayfirst=True,errors="coerce")

def get_date_column(df:pd.DataFrame, prefer:list[str]):
    for col in prefer:
        if col in df.columns:
            df[col]=parse_dates_series(df[col]); return col
...
def extract_csv_from_zip(zfile):
    try:
        with zipfile.ZipFile(zfile) as z:
            ...
            if not csvs: st.error("❌ ZIP tidak memuat CSV."); return pd.DataFrame()
            dfs=[pd.read_csv(z.open(f),delimiter=";",quotechar='"',
                             engine="python",on_bad_lines="skip",dtype=str) for f in csvs]
            return pd.concat(dfs,ignore_index=True)
    except zipfile.BadZipFile:
        st.error("❌ ZIP rusak."); return pd.DataFrame()
...
def badge(s): return f'<span style="color:{ {"positive":"green","negative":"red","neutral":"gray"}.get(str(s).lower(),"black") };font-weight:bold">{s}</span>'

# ---------- UTIL : advanced keywords ----------
def parse_adv(q):
    q=q.strip(); inc,phr,exc=[],[],[]
...
def run_onm(df):
    ...
    if mode=="Custom":
        if date_col and not df[date_col].dropna().empty:
            ...
            d_start,d_end=st.sidebar.date_input("Rentang",(mn,mx),min_value=mn,max_value=mx,key="onm_date_input")
            use_date=True
        else: st.sidebar.info("Kolom tanggal tidak valid.")
    ...
    if kw: m &= (df["title"].apply(lambda x: match(x,inc,phr,exc)) |
    ...
    if hl:
        pattern=re.compile("(?i)("+"|".join(map(re.escape,[h.strip('"') for h in re.findall(r'"[^"]+"|\S+', hl)]))+")")
        gpd["title"]=gpd["title"].apply(lambda t: pattern.sub(r"<mark>\1</mark>", t))
    st.markdown("### 📊 Ringkasan Topik (ONM)")
    st.write(gpd[["title","Total","Sentiment","Link"]].style
             .set_properties(subset=["Total","Link"],**{"text-align":"center"})
...
def run_sosmed(df):
    ...
    # --- pilih kolom platform yang tersedia
    if "specific_resource_type" in df.columns and df["specific_resource_type"].str.strip().any():
        plat_col="specific_resource_type"
    else:
        plat_col="specific_resource"
    
    st.sidebar.header("🎛️ Filter — Sosmed")
    scope=st.sidebar.radio("Conversation",("All","Tanpa Comment","Tanpa Post"),index=0)
    # PLATFORM check-box horizontal
    ...
    if plat_values:
        ...
        for idx,p in enumerate(plat_values):
            label_html=platform_color_badge(p)
            default=True
            if cols[idx].checkbox(label=p,key=f"plat_{p}",value=default,label_visibility="collapsed"):
                sel_plat.append(p)
            cols[idx].markdown(label_html,unsafe_allow_html=True)
    else:
        sel_plat=[]
    ...
    if mode=="Custom":
        if date_col and not df[date_col].dropna().empty:
            mn,mx=df[date_col].min().date(),df[date_col].max().date()
            d_start,d_end=st.sidebar.date_input("Rentang",(mn,mx),min_value=mn,max_value=mx,key="soc_date_input")
            use_date=True
        else: st.sidebar.info("Kolom tanggal tidak valid.")
    ...
    summary["Short"]=summary["content"].apply(lambda t:textwrap.shorten(re.sub(r'<.*?>','',str(t)),120,"…"))
    ...
    summary["Platform"]=summary["Link"].apply(
        lambda html: platform_from_url(re.search(r'href="([^"]+)"',html).group(1)) if "href" in html else "-")
...
if st.session_state.df is not None:
    ...
    else: st.error("❌ Struktur kolom tidak cocok (ONM/Sosmed).")
else:
