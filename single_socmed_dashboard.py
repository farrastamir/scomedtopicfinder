...
def word_freq(series, top=500):
    tokens = re.findall(r"\b\w{3,}\b", " ".join(series).lower())
    tokens = [t for t in tokens if t not in STOPWORDS]
    return Counter(tokens).most_common(top)

def badge(s):
    return f'<span style="color:{ {"positive":"green","negative":"red","neutral":"gray"}.get(str(s).lower(),"black") };font-weight:bold">{s}</span>'
...
def run_onm(df):
    ...
    if mode == "Custom":
        if date_col and not df[date_col].dropna().empty:
            ...
            st.sidebar.info("Kolom tanggal tidak valid.")
...
def run_sosmed(df):
    need = {"content", "post_type", "final_sentiment",
            "specific_resource", "specific_resource_type",
            "reply_to_original_id", "original_id",
            "link", "date_created"}
    df = trim_columns(df.rename(str.lower, axis=1), need)
    ...
    # checkbox platform horizontal
    plat_values = sorted({p for p in df[plat_col].dropna() if str(p).strip()})
    sel_plat = []
    if plat_values:
        st.sidebar.markdown("**Platform**")
        cols_plat = st.sidebar.columns(len(plat_values))
        for i, p in enumerate(plat_values):
            html = platform_color_badge(p)
            if cols_plat[i].checkbox(p, value=True, key=f"plat_{p}", label_visibility="collapsed"):
                sel_plat.append(p)
            cols_plat[i].markdown(html, unsafe_allow_html=True)

    sent_sel = st.sidebar.selectbox("Sentimen",
                                    ["All"] + sorted(df["final_sentiment"].str.lower().unique()))
    ...
    summary["Short"] = summary["content"].apply(
        lambda t: textwrap.shorten(re.sub(r'<.*?>', '', str(t)), 120, "…"))
    summary["Text"] = summary.apply(
        lambda r: f'<span title="{str(r.content)}">{r.Short}</span>', axis=1)
