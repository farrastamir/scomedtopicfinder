...
def run_onm(df, model):
    ...
    if st.session_state.selected_onm_title:
        ...
        def show_news_popup(title_to_show):
            ...
            with col1:
                ...
                st.markdown("---")
                with st.container(height=400): st.markdown(body_display, unsafe_allow_html=True)
            with col2:
    ...
    else: st.info("Tidak ada data topik untuk ditampilkan sesuai filter yang dipilih.")
    if show_wc:
...
def run_sosmed(df, model):
    ...
    if plat_values:
        ...
        num_cols = min(len(plat_values), 4)
        cols = st.sidebar.columns(num_cols)
        for i, p in enumerate(plat_values):
            col_idx = i % num_cols
            with cols[col_idx]:
                if st.checkbox(p, True, key=f"plat_{p}", label_visibility="collapsed"): sel_plat.append(p)
...
if data_source:
    with st.spinner("📖 Membaca dan mengonversi data ke Parquet..."):
        ...
        try:
            ...
            if file_name.lower().endswith(".zip"): df_all = extract_csv_from_zip(data_source)
            elif file_name.lower().endswith(".csv"):
                df_all = pd.read_csv(data_source, delimiter=";", quotechar='"',
                                       engine="python", on_bad_lines="skip", dtype=str)
            if not df_all.empty:
                parquet_buffer = io.BytesIO()
                df_all.to_parquet(parquet_buffer, engine='pyarrow')
                st.session_state.parquet_buffer = parquet_buffer
                st.success(f"✅ Data berhasil diproses ({len(df_all):,} baris). Siap untuk dianalisis.")
            else: st.session_state.parquet_buffer = None
        except Exception as e:
...
if st.session_state.parquet_buffer is not None:
    ...
    buffer.seek(0)
    df_raw = pd.read_parquet(buffer, engine='pyarrow')
